...
describe('ProcessPayment', () => {
  ...
  it(
    'should not be able to process a payment of a non-existing ride',
    async () => {
      const processPayment = new ProcessPayment();
  ...
  it(
    'should not be able to process a payment of a non-completed ride',
    async () => {
      const registry = Registry.getInstance();
      ...
      await expect(
        processPayment.execute(input)
      ).rejects.toThrow(new ConflictException('ride not finished yet'));
    });
});
