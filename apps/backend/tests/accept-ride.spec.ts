...
describe('POST /rides/:rideId/:driverId', () => {
  ...
  it(
    'should not be able to accept a ride when driver already have an active ride',
    async () => {
      ...
      await makeRequest('POST', `/v1/rides/${ride1Id}/${driverId}`);
      const acceptRideRes = await makeRequest<Object>(
        'POST',
        `/v1/rides/${ride2Id}/${driverId}`
      );

      expect(acceptRideRes.status).toBe(409);
      expect(
        acceptRideRes.data?.message
      ).toBe('driver already have an active ride');
    });
});
